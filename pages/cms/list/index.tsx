...
import { LandingPage } from "@/Api/Functions/list.api";
...
console.log(tp,"totalpage")
  ...
  const filterprod = prod?.filter((product:{title:string,description:string,image:File}) =>
  ...
  return (
    <>
      {isOnline ? (
        <>
          <div style={{display:"flex",justifyContent:"center",alignItems:"center",margin:"40px"}} className="flex justify-center items-center  m-3">
            <input
              className="p-1 py-2 width:[40%] bg-slate-200 rounded-xl hover:shadow-lg hover:shadow-slate-300"
              type="text"
              placeholder="Search here"
              value={search}
              style={{padding:"16px",paddingTop:"32px",paddingBottom:"32px",width:"15%",backgroundColor:"white",borderRadius:"10px"}}
              onChange={(e) => {
